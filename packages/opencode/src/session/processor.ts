  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        Log.Default.info("\n\n1、--- SYSTEM ---")
        streamInput.system.forEach((s, i) => Log.Default.info(`[${i}] ${s.slice(0, 3000)}${s.length > 3000 ? "..." : ""}`))
        
        Log.Default.info("\n\n2、--- MESSAGES ---")
        streamInput.messages.forEach((m, i) => {
          const role = "role" in m ? (m as any).role : "unknown"
          const content = "content" in m ? (m as any).content : JSON.stringify(m).slice(0, 1500)
          const contentStr = typeof content === "string" ? content.slice(0, 2000) : JSON.stringify(content).slice(0, 2000)
          Log.Default.info(`[${i}] ${role}: ${contentStr}${contentStr.length >= 2000 ? "..." : ""}`)
        })
        
        Log.Default.info("\n\n3、--- TOOLS ---")
        Log.Default.info(`Total: ${Object.keys(streamInput.tools).length} tools`)
        Log.Default.info("================================\n")

        Log.Default.info("================================================ 三、STREAM INPUT ==================================================")
        Log.Default.info(JSON.stringify(streamInput, null, 2))
        Log.Default.info("=================================================================================================================")
