      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            Log.Default.info("\n\n================================================== 一、BUILD CONTEXT ==================================================")
            Log.Default.info("\n\n--- 1. ENV (环境信息) ---")
            env.forEach((e, i) => Log.Default.info(`[${i}] ${e.slice(0, 2000)}${e.length > 2000 ? "..." : ""}`))
            Log.Default.info("\n\n--- 2. INSTRUCTIONS (指令) ---")
            instructions.forEach((e, i) => Log.Default.info(`[${i}] ${e.slice(0, 2000)}${e.length > 2000 ? "..." : ""}`))
            Log.Default.info("\n\n--- 3. SKILLS ---")
            Log.Default.info(skills ?? "(none)")
            Log.Default.info("\n\n--- 4. MODEL MSGS (历史消息) ---")
            Log.Default.info(`Total: ${modelMsgs.length} messages`)
            modelMsgs.forEach((m, i) => {
              const content =
                typeof m.content === "string" ? m.content.slice(0, 1000) : JSON.stringify(m.content).slice(0, 1000)
              Log.Default.info(`[${i}] ${m.role}: ${content}${content.length >= 1000 ? "..." : ""}`)
            })
            Log.Default.info("\n\n--- 5. TOOLS ---")
            Log.Default.info(`Total: ${Object.keys(tools).length} tools: ${Object.keys(tools).join(", ")}`)
            Log.Default.info("========================================================================================================================\n")
